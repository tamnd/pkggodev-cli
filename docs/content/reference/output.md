...
describes what users get. Pick a format with `-o`, or let gopkg choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
gopkg <command> -o table   # aligned columns for reading
gopkg <command> -o jsonl   # one JSON object per line, for piping
gopkg <command> -o json    # a single JSON array
gopkg <command> -o csv     # spreadsheet friendly
gopkg <command> -o tsv     # tab-separated
gopkg <command> -o url     # just the URL column
gopkg <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
gopkg <command> --fields id,title,url
```
...
```bash
gopkg <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
gopkg <command>            # a table, because this is a terminal
gopkg <command> | wc -l    # JSONL, because this is a pipe
```
