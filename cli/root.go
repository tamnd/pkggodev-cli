// Package cli builds the gopkg command tree on top of the pkggodev library.
package cli
...
import (
	...
	"github.com/tamnd/pkggodev-cli/pkggodev"
)
...
func Root() *cobra.Command {
	app := &App{cfg: pkggodev.DefaultConfig()}

	root := &cobra.Command{
		Use:   "gopkg",
		Short: "Browse Go packages on pkg.go.dev",
		Long: `gopkg reads Go package and module data from pkg.go.dev, proxy.golang.org,
and sum.golang.org. All three are open and require no API key. It returns
records as table, JSON, JSONL, CSV, TSV, or URLs.

gopkg is an independent tool and is not affiliated with Google or the Go team.`,
		SilenceUsage:  true,
		SilenceErrors: true,
...
func (a *App) setup() error {
	...
	a.client = pkggodev.NewClient(a.cfg)
	return nil
}
